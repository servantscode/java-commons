...
public class InsertBuilderTest {
    ...
    public void testInsertBuilderSelectValuesAfterSelectFailure(){
        InsertBuilder cmd = new InsertBuilder().into("activity")
                .select(new QueryBuilder().select("activity.id, p.id, false"))
                .value("col_1", 7);
    }

    @Test(expected = IllegalStateException.class)
    public void testInsertBuilderSelectAfterValuesFailure(){
        InsertBuilder cmd = new InsertBuilder().into("activity")
                .value("col_1", 7)
                .select(new QueryBuilder().select("activity.id, p.id, false"));
    }
    ...
    public void testInsertBuilderWhere(){
        InsertBuilder cmd = new InsertBuilder().into("activity_read_status")
                ...
                .select(new QueryBuilder().select("activity.id, p.id, FALSE")
                                          ...
                                          .where("activity.id =?", 6))
                .onConflict("(activity_id, person_id) DO NOTHING");
